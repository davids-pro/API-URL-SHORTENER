...
/**
 * retourne un id de type string sur 6 caractères
 * 62^6 = 56 800 235 584 combinaisons uniques possibles
 */
const idGenerator = () => {
  let id = '';
  // 26 majuscules, 26 minuscules et 10 chiffres possibles dans l'id généré
...
const addHttpProtocol = (url) => {
  return /^(http:\/\/|https:\/\/|ftp:\/\/|sftp:\/\/|ssh:\/\/|data:image)/
    .test(url) ? url : 'http://' + url;
};
...
/**
 * génère un document avec un id aléatoire
 */
...
/**
 * génère un document avec un id choisi par l'utilisateur
 */
